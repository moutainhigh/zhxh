...
import net.ussoft.zhxh.base.BaseDao;
import net.ussoft.zhxh.base.DaoException;
import net.ussoft.zhxh.model.PageBean;
import net.ussoft.zhxh.util.BeanUtilsBean;
...
//		return this.getJdbcTemplate().update(sql, values.toArray());
	...
	@Override
	public int del(ID id,String tablename){
		if(id==null)return 0;
		StringBuilder sb = new StringBuilder("delete from ");
		sb.append(tablename);
		sb.append(" where ");
...
//		sb.append("select count(*) from ").append(this.tableName).append(" ").append(sql);
	...
	protected int updateById(Map<String, Object> map,String tablename){
		if(map==null||map.size()==0)return 0;
		Serializable id=(Serializable) map.get(this.getPk());
		if(id==null||"".equals(id))return 0;
		List<Object> values=new ArrayList<Object>();
	...
	@Override
	public ID save(Map<String,Object> m,String tablename) {
		if(m==null){
			throw new DaoException("模型类内容为空！");
		}
		...
		ID id=this.addReturnId(m, tablename);
		
		return id;
	}
	
	@SuppressWarnings("unchecked")
	protected ID addReturnId(Map<String, Object> map,String tablename){
		if(map==null||map.size()==0)return null;
		if(strategy.equals(GenerationType.IDENTITY)){
			map.remove(this.getPk());
		}
	...
	@Override
	public int update(Map<String,Object> m,String tablename) {
		
		int count=this.updateById(m,tablename);
		if(count==0){
