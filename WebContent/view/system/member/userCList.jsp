...
<head>
    ...
    <script type="text/javascript">
	    		...
	    		columns: [
							{ type: "checkcolumn",headerAlign:"center",width: 30},
	      	                ...
	      	                { field: "companycode",name:"companycode",width: 100, headerAlign: "center", align:"center",allowSort: false, header: "机构代码"},
	      	              	...
	      	              	{ field: "tuijianman",name:"tuijianman",width: 180, headerAlign: "center", align:"center",allowSort: false, header: "推荐人"},
	    ...
	    function addRow(grid_type) {
			...
			if (grid_type == "grid_user_z") {
				tmpGrid = grid_user_z;
				
				var rows = grid_user_c.getSelecteds();
				var companycode = "";
	       	 	if (rows.length == 0) {
	       	 		parent.parent.layer.msg("请选择要添加普通会员的所属门店.",{icon:6});
	       		 	return;
	       	 	}
	       	 	else {
	       	 		if (typeof(rows[0].id) == "undefined" || rows[0].id == "") {
	       	 			parent.parent.layer.msg("当前门店还没有保存。请先保存后再创建.",{icon:6});
	       	 			return;
	       	 		}
	       	 		else {
      ...
      //查看代理下的店、会员
        function onSelectionChanged(e) {
      	...
      	function outLink(gridtype) {
			...
			var tmpGrid;
      		...
      		var rows = tmpGrid.getSelecteds();
       	 	...
       	 	var cf1 = "确定要解除普通会员与门店的关系吗？<br><p style='font-size:12px; color:red'>注意：解除关系后，普通会员将进入[待关联机构]查看，请谨慎操作.</p>";
       	 	var rows_c = grid_user_c.getSelecteds();
       	 	var ids = getSelectGridid(rows);
       	 	parent.parent.layer.msg(cf1, {
    	 		icon:3
    	 		,time: 0 //不自动关闭
    	  		...
    	  		,yes: function(index){
        	        $.ajax({
        	            ...
        	            data: {'parentid':rows_c[0].id,'userids':ids},
        	            type: "post",
        	            dataType:"text",
        	            success: function (text) {
        	            	if (text == "success") {
      	...
      	function move(gridtype){
        	...
        	var rows_c = grid_user_c.getSelecteds();	//所选行数 多行
            if (rows_c.length == 1) {
            	var oldParentid = rows_c[0].id;
            	var newParentid = "";
                mini.open({
                	url: "${pageContext.request.contextPath}/common/dispatch.htmls?page=/view/system/member/selectUser",
	    	...
	    	<div class="mini-toolbar" style="padding:0px;border-top:0;border-left:0;border-right:0;border-bottom:1;">
		        <table style="width:100%;">
		            <tbody>
		             <tr>
		                 <td style="width:100%;">
		                 	<span id="pid" style="padding-left:5px;">全部门店列表</span>
		                 </td>
