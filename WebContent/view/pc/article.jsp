...
<body>
	...
	<div class="HTML-con case">
	    ...
	    <div class="case-box new" style="display:block">
	        <c:forEach var="item" items="${articleList }">
		    	<dl class="clearfix">
		            ...
		            <dd class="fr">
		                ...
		                <p>${item.brief }</p>
		                <a href="${pageContext.request.contextPath}/pcMain/article-c.htmls?id=${item.id }" target="_blank">了解更多</a>
		            </dd>
		        </dl>
		    </c:forEach>
	        ...
	        <!--分页-->
	        <ul class="page">
            	<li><a href="javascript:window.location.href='${pageContext.request.contextPath}/pcMain/article.htmls?ptype=${ptype }&page=1'" class="${page == 1?'cur':'' }">1</a></li>
				<c:if test="${pageCount != 1}">  
				    <c:choose>  
				        <c:when test="${page <= 5}">  
				            <c:forEach var="i" begin="2" end="${page}">  
				                <li><a href="javascript:window.location.href='${pageContext.request.contextPath}/pcMain/article.htmls?ptype=${ptype }&page=${i }'" class="${page == i?'cur':'' }">${i }</a></li>
				            </c:forEach>
				        </c:when>  
				        <c:otherwise>
				        	<li id="" >...</li>
				    ...
				    <c:choose>
				        ...
				        <c:otherwise>  
				            ...
				            <li id="" >...</li>
				            <li><a href="javascript:window.location.href='${pageContext.request.contextPath}/pcMain/article.htmls?ptype=${ptype }&page=${pageCount }'" class="${page == i?'cur':'' }">${pageCount }</a></li>
				        </c:otherwise>  
				    </c:choose>  
				</c:if>
